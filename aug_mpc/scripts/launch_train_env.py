...
if __name__ == "__main__":  
    ...
    parser.add_argument('--q_norm_mode', type=str, help="SAC: actor-loss Q normalization. 'none' (legacy) or 'actor_rms' (divide min_q in the actor loss by an EMA of its RMS, decoupling alpha from the Q scale / discount factor).", default='none')
    parser.add_argument('--q_norm_beta', type=float, help="SAC: EMA rate of the running Q scale for q_norm_mode=actor_rms", default=5e-3)
    parser.add_argument('--disc_entropy_mode', type=str, help="HYBRID SAC: discrete-branch entropy handling. 'off' (alpha_disc==0, no discrete entropy in the objective; deterministic discrete target -- explore via extrinsic flip envs), 'auto' (autotune alpha_disc to target_H_disc_frac), 'fixed' (constant alpha_disc at its init).", default='off')
