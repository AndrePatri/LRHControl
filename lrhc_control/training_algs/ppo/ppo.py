...
class PPO():
    ...
    def __init__(self,
        ...
        # Implementation of PPO built starting from cleanrl's one. Compared with clearl one, it
        # properly handles bootstrapping at terminations and truncations and it does so by storing 
        # current and next observations in separate buffers, which greatly improves code readability at
        # the expense of memory efficient (basically, it requires double the memory for obs buffer compared
        # to a minimalistic, but more convolute, implementation)
