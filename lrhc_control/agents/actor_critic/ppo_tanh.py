    ...
    def __init__(self,
        ...
        if self._normalize_obs:
            self.critic = nn.Sequential(
                ...
                nn.Tanh(),
                self._layer_init(layer=nn.Linear(64, 64), device=self._torch_device,dtype=self._torch_dtype),
                nn.Tanh(),
                self._layer_init(layer=nn.Linear(64, 1), std=self._critic_std, device=self._torch_device,dtype=self._torch_dtype),
            ) # (stochastic critic)
